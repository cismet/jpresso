...
/** 
 * 
 * Hauptklasse des ganzen Paketes. Hier laufen alle Faeden zusammen ;-)
 *
 * @author srichter
...
public final class Importer implements FinalizerCreator, ExtractAndTransformController {
     ...
     * 
     * @param impRules
     * @throws de.cismet.cids.admin.importAnt.InitializingException
     */
    public Importer(final ImportRules impRules, final ClassResourceProvider clp) throws InitializingException {
        ...
        try {
            ...
            throw new InitializingException("Error while creating meta information! (" + miEx.getMessage() + ")", initializeLog.toString(), miEx);
            ...
            throw new InitializingException("Error creating internal datastructure! " + wnEx.getMessage(), initializeLog.toString(), wnEx);
        } catch (DynamicCompilingException dcEx) {
            throw new InitializingException("Error creating the mapping class! " + dcEx.getMessage(), initializeLog.toString(), dcEx);
        } catch (InitializingException ie) {
            throw new InitializingException(ie.getMessage(), ie.getInitializeLog(), ie.getMappingErrorFields(), ie.getReferenceErrorFields());
        }
    }

    public final FinalizerCreator runImport() throws JPressoException {
     ...
     * 
     * - Empty mapping content
     * - Relationpath errors
     * - AutoInc-, Compare- an Enclosingcharacter issues
     * 
     * @param rules
     */
...
//        final Set<String> possibleRelationTables = new HashSet<String>();
        ...
        for (final Mapping m : mappings) {
            ...
            if (m.getContent().length() < 1 && !m.isAutoIncrement()) {
                //leerer inhalt
                msg = "Mapping-Error: Emtpy mapping for " + m.getCompleteTarget() + "! Tablepos.: Row " + row + ", Column " + 1;
                log.error(msg);
            ...
            if (m.getTargetField().length() > 0 && m.getTargetTable().length() > 0) {
                ...
                if (m.isAutoIncrement() && m.isComparing()) {
                    ...
                    msg = "Mapping-Error: AutoIncrement AND Comparing activated for " + m.getCompleteTarget() + "! Tablepos.: Row " + row + ", Column " + 3 + " and " + 4;
                    log.error(msg);
                    sb.append(msg).append("\n");
                ...
                try {
                    ...
                    if (!fields.add(m.getTargetField())) {
                        ...
                        msg = "Mapping-Error: Target " + m.getCompleteTarget() + " used more than once! Tablepos.: Row " + row + ", Column " + 2;
                        log.error(msg);
                    ...
                    msg = "Mapping-Error: Relationpath " + m.getPath() + " on target " + m.getCompleteTarget() + " is not a number! Tablepos.: Row " + row + ", Column " + 2;
                    log.error(msg);
                    mappingErrors.add(new Point(row, 2));
                    sb.append(msg).append("\n");
                }
        ...
        for (final Mapping m : mappings) {
            ...
            if (seenWithPath.contains(m.getTargetTable()) && m.getPath().length() < 1) {
                ...
                msg = "Mapping-Error: Target " + m.getCompleteTarget() + " found with and without paths! Tablepos.: Row " + row + ", Column " + 2;
                log.error(msg);
                sb.append(msg).append("\n");
                mappingErrors.add(new Point(row, 2));
            }
            if (autoInc.contains(key) && !m.isAutoIncrement()) {
                msg = "Mapping-Error: Target " + m.getCompleteTarget() + " has different Auto-Increment-settings among it's paths! Tablepos.: Row " + row + ", Column " + 3;
                log.error(msg);
                sb.append(msg).append("\n");
                mappingErrors.add(new Point(row, 3));
            }
            if (compares.contains(key) && !m.isComparing()) {
                msg = "Mapping-Error: Target " + m.getCompleteTarget() + " has different Comparing-settings among it's paths! Tablepos.: Row " + row + ", Column " + 4;
                log.error(msg);
                sb.append(msg).append("\n");
                mappingErrors.add(new Point(row, 4));
            }
            ...
            if (encChar != null && !encChar.equals(m.getEnclosingChar())) {
                msg = "Mapping-Error: Target " + m.getCompleteTarget() + " has different Enclosing-Character-settings among it's paths! Tablepos.: Row " + row + ", Column " + 5;
                log.error(msg);
                sb.append(msg).append("\n");
        ...
        for (final Reference ref : references) {
            if (ref.isComparing()) {
                ...
                final String pureCompleteReferencing = (cut > 0 ? ref.getReferencingTable().substring(0, cut) : ref.getReferencingTable()) + "." + ref.getReferencingField();
        ...
        for (final Reference ref : references) {
            ...
            if (!findDuplicates.add(ref.getCompleteReferencing())) {
                msg = "Referencing-Error: Target " + ref.getCompleteReferencing() + " is already assigned in another Relation! Tablepos.: Row " + row + ", Column " + 0 + " and " + 1;
                log.error(msg);
                sb.append(msg).append("\n");
            ...
            if (mappingTestSet != null) {
                if (mappingTestSet.contains(ref.getReferencingField())) {
                    msg = "Mapping/Referencing-Error: Target " + ref.getCompleteReferencing() + " is already assigned in a Mapping! Tablepos.: Row " + row + ", Column " + 0 + " and " + 1;
                    log.error(msg);
                    sb.append(msg).append("\n");
                    referenceErrors.add(new Point(row, 0));
                    referenceErrors.add(new Point(row, 1));
            ...
            if (fromFields == null) {
                ...
                msg = "Referencing-Error: Mapping does not contain the Referencing-Table " + ref.getReferencingTable() + "! Tablepos.: Row " + row + ", Column " + 0;
                log.error(msg);
                sb.append(msg).append("\n");
                referenceErrors.add(new Point(row, 0));
            }
            if (toFields == null) {
                ...
                msg = "Referencing-Error: Mapping does not contain the Referenced-Table " + ref.getReferencedTable() + "! Tablepos.: Row " + row + ", Column " + 2;
                log.error(msg);
                sb.append(msg).append("\n");
            ...
            if (compares.contains(pureCompleteReferencing) && !ref.isComparing()) {
                ...
                msg = "Mapping/Referencing-Error: Found different setting for Comparing on table " + pureCompleteReferencing + " Tablepos.: Row " + row + ", Column " + 4;
                log.error(msg);
                sb.append(msg).append("\n");
            ...
            if (encChar != null && !encChar.equals(ref.getEnclosingChar())) {
                ...
                msg = "Mapping/Referencing-Error: Found different setting for Enclosing-Character on table " + pureCompleteReferencing + "! Tablepos.: Row " + row + ", Column " + 5;
                log.error(msg);
                sb.append(msg).append("\n");
        ...
        if (mappingErrors.size() > 0 || referenceErrors.size() > 0) {
            throw new InitializingException("Found errors when prechecking Mappings and References! Task canceled.", sb.toString(), mappingErrors, referenceErrors);
        }
    }
     ...
     */
    private final void memCheck(final long min) {
     ...
     */
    @Override
    public final FinalizerCreator runImport(final ProgressListener progress) throws JPressoException {
        if (closed) {
    ...
    /** 
     * Fuehrt auf der Zieldatenbank Kontroll-Statements aus, um zu ueberpruefen ob die
    ...
    public String[] getEnclosingCharacters(TableModel table) {
        if (table instanceof IntermedTable) {
            IntermedTable iTab = (IntermedTable) table;
