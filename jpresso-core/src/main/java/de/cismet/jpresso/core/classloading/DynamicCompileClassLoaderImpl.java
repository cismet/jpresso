...
/**
 * A classloader that can compile source to classes and load them into the jvm
 * It uses an array of resource-urls as classpath extention.
 * 
 * The delegation can be switched between child-first and parent-first delegation
 * by setting the parentFirstDelegation flag (default: is true).
 * 
 * A filter can be set, to apply the opposite delegation on classes matching the filter.
 * 
 * @author srichter
 */
...
//    private static final String MASKED_FILE_SEPERATOR = "\\" + File.separator;
     ...
     * @param urls
     */
    private DynamicCompileClassLoaderImpl(DynamicCompileClassLoaderImpl instanceToCopy) {
        //Make the classloader child of the current ContextClassloader to make other loaded Classes visible
        //to the classes we are going to load!
        this(instanceToCopy.getURLs(), instanceToCopy.getParent(), instanceToCopy.baseDir, instanceToCopy.parentFirstDelegation, instanceToCopy.filter != null ? instanceToCopy.filter.pattern() : null);
    }
     ...
     * @param parent
     */
    DynamicCompileClassLoaderImpl(final URL[] urls, final ClassLoader parent, final String baseDir, final boolean parentFirstDelegation, final String delegationFilterRegex) {
        super(urls, parent);
        //further checks...
        if (urls == null) {
     ...
     * @return
     */
    private synchronized Class<?> getTheClass(final String className, final CompilerResult compRes) throws ClassNotFoundException {
    ...
    /**
     * Changed delegation for Class AssignerBase.
     * These are loaded with child-first, all others are loaded as parent-first.
     * This creates thread-safe static AssignerBases, which are only visible to 
     * the appropriate Assigner.
     * 
     * DANGER: EVIL HACK!
     * 
     * @param name
...
//            final String className = sourceFile.getAbsolutePath().substring(baseDir.length() + 1, sourceFile.getAbsolutePath().length() - (JPressoFileManager.END_JAVA.length() + 1)).replaceAll(MASKED_FILE_SEPERATOR, DOT);
     ...
     */
    @Override
    public <T> Class<? extends T> compileAndLoadClass(final String className, final String code, Class<T> clazz) throws DynamicCompilingException {
        return compileAndLoadClass(new JavaClassDefinition(className, code)).asSubclass(clazz);
    }

    /**
     *  Fix for strange behaviour of the Java Compiler API: Changed classes were
     *  no longer automatically recompiled, as long as any class with a date
     *  newer than its source-file exists. So this fix sets the timestamps of
     *  all java-sources in the project to the timestamp of the newest.
     *  WARNING: directories are JPresso-specific!
     */
    private final void fixRecompile() {
     ...
     * @throws de.cismet.jpressocore.serviceprovider.exceptions.DynamicCompilingException
     */
    private synchronized Class<?> compileAndLoadClass(JavaClassDefinition source) throws DynamicCompilingException {
        ...
        try {
            ...
            if (compRes.isSuccessful()) {
                ...
                final DynamicCompilingException dce = new DynamicCompilingException(errorOutStream.toString(), pos, det, codeView);
            ...
            throw new DynamicCompilingException("Internal Compilation Bug: " + ex.toString(), createSourceCodeView(source));
        } finally {
            System.setErr(System.out);
        }
    }

    private final String createSourceCodeView(final JavaClassDefinition jcd) {
