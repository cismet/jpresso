 ...
 * Utility class.
 * 
 * Copies all referenced drivers and external libs into the project directory
 * and creates a modified options.jpo and build.xml with hardcoded values for
 * usage with external ant.
 * 
 * @author srichter
 */
public abstract class ImporterExporter {
    ...
    /**
     * Exports the project to a single zip file.
     * As Java zip API sucks, we first have to collect all necessary files into
     * our project directory. Therefore all libs and driver jars in the 
     * options.jpo are looked up and copied into the project directory. They
    ...
    /**
     * Imports the project from a previously created zip.
     * After unzipping, the options.jpo is adapted to the new project directory,
     * regarding all absolute paths.
     * 
     * @param importFile
     * @param dest
     * @throws java.io.IOException
    ...
    public static final void importProjectFromZip(final File importFile, final File dest) throws IOException {
        ...
        if (!importFile.isFile()) {
            throw new IllegalStateException("The given import file " + importFile.getAbsolutePath() + " is not a file or does not exist!");
        }
        if (!dest.isDirectory()) {
            throw new IllegalStateException("The given destination file " + dest.getAbsolutePath() + " is not a directory or does not exist!");
        }
    ...
    public static final void restoreProjectEnvironment(final File destDir) throws IOException {
        String msg;
        final JPressoFileManager manager = JPressoFileManager.getDefault();
        File impOpts = new File(destDir, JPressoFileManager.IMPORT_OPTIONS);
        if (!impOpts.isFile()) {
            throw new IllegalStateException("The file you try to import is not a valid JPresso Project! Can not find " + JPressoFileManager.IMPORT_OPTIONS);
        }
        ...
        for (final File cur : oldcpf) {
            ...
            File toProcess = new File(cur.getAbsolutePath().replace("\\", File.separator).replace("/", File.separator));
            newcpf.add(new File(destDir.getAbsolutePath() + File.separator + JPressoFileManager.DIR_LIB + File.separator + toProcess.getName()));
        ...
        for (final DriverDescription dd : po.getDriver()) {
            for (int i = 0; i < dd.getJarFiles().size(); ++i) {
                ...
                final File newF = new File(destDir.getAbsolutePath() + File.separator + JPressoFileManager.DIR_DRV + File.separator + oldF.getName());
    ...
    /**
     *  Copies a Set of Files to a destination directory.
     *  If the destination or directory file does not exist, it is created.
     *  If two files would have the same name, a similar name is chosen for th second file.
     * 
     * @param files
     * @param destDir
     ...
     * @throws java.io.IOException
     */
    public static final Map<File, File> copyFilesToDirWithAutomaticRename(final Set<File> files, final File destDir) throws IOException {
        final JPressoFileManager manager = JPressoFileManager.getDefault();
